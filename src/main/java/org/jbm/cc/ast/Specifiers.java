 ...
 * specifiers and qualifiers fold to. type is null only when {@code auto}
...
public record Specifiers(Token token, List<Token> storageClasses, List<Token> functionSpecifiers,
                         Alignas alignment, Type type) {

    /** alignment-specifier (6.7.6). Exactly one of type / expr is non-null. */
    public record Alignas(Token keyword, Type type, Expr expr) {
    }
